...
def render_general_journal():
    ...
    st.markdown("Record journal entries for business transactions.")
    ...
    with col_d2:
        entry_desc = st.text_input("Description", placeholder="e.g., Paid rent for January")

    # Entry lines (at least 2: debit + credit)
    st.markdown("**Entry Lines** — each debit must equal total credits.")

    num_lines = st.number_input("Number of lines", min_value=2, max_value=8, value=2, step=1,
                                help="A journal entry needs at least 2 lines (debit + credit).")
    ...
    cols[1].markdown("**Debit (Rp)**")
    cols[2].markdown("**Credit (Rp)**")

    for i in range(int(num_lines)):
        cols = st.columns([3, 2, 2, 0.5])
        with cols[0]:
            acc = st.selectbox(f"Account {i+1}", available_accounts, key=f"je_acc_{i}", label_visibility="collapsed")
        with cols[1]:
            deb = st.number_input(f"Debit {i+1}", min_value=0, value=0, step=10000, format="%d", key=f"je_db_{i}", label_visibility="collapsed")
        with cols[2]:
            cred = st.number_input(f"Credit {i+1}", min_value=0, value=0, step=10000, format="%d", key=f"je_cr_{i}", label_visibility="collapsed")
    ...
    if total_debit > 0 or total_credit > 0:
        ...
        if abs(diff) < 1:
            ...
            st.error(f"❌ Unbalanced! Debit Rp {total_debit:,.0f} vs Credit Rp {total_credit:,.0f} (diff: Rp {abs(diff):,.0f})")

    # Post button
    if st.button("📌 Post Entry", type="primary", disabled=abs(total_debit - total_credit) > 0.5 or total_debit == 0):
    ...
    if not st.session_state.journal_entries:
        st.info("No journal entries yet. Post your first entry above!")
    else:
        # Filter by company type
        filtered = [e for e in st.session_state.journal_entries if e.get("company_type") == company_type]
        if not filtered:
            st.info(f"No entries for {company_type} company. Switch type or post entries.")
            ...
            with st.expander(f"**{ref}** — {entry['date']} | {entry['description']}", expanded=False):
                ...
                for line in entry["entries"]:
                    rows.append({
                        ...
                        "Debit (Rp)": f"Rp {line['debit']:,.0f}" if line['debit'] > 0 else "",
                        "Credit (Rp)": f"Rp {line['credit']:,.0f}" if line['credit'] > 0 else "",
                    })
                df = pd.DataFrame(rows)
                st.dataframe(df, width="stretch", hide_index=True)

                # Delete single entry
                if st.button(f"Delete {ref}", key=f"del_{ref}"):
                    st.session_state.journal_entries = [
...
def render_general_ledger():
    ...
    st.markdown("View T-accounts and running balances posted from the General Journal.")
    ...
    if not filtered:
        st.info("No journal entries. Go to **General Journal** to post entries first.")
        return

    # Build ledger by account
    ledger = {}  # account_name -> list of (date, ref, desc, debit, credit)
    ...
    # Company type info
    st.info(f"**Company Type:** {company_type} | **Total Accounts:** {len(ledger)}")

    # Select account to view
    account_names = sorted(ledger.keys())
    selected_account = st.selectbox("Select Account", account_names)

    if selected_account:
        st.subheader(f"📋 {selected_account}")

        entries = ledger[selected_account]
        acc_type = get_account_type(selected_account, company_type)
        normal_side = get_account_normal_balance(acc_type)

        st.caption(f"Classification: **{acc_type}** | Normal Balance: **{normal_side}**")
        ...
        for e in entries:
            ...
            rows.append({
                "Date": e["date"],
                "Ref": e["ref"],
                "Description": e["desc"][:40] + "..." if len(e["desc"]) > 40 else e["desc"],
        ...
        end_balance = total_debits - total_credits

        col_t1, col_t2 = st.columns(2)

        with col_t1:
            st.markdown(f"""<div style='background:#1a2a1a; border:1px solid #3a6a3a; border-radius:8px; padding:12px; margin-bottom:8px;'>
                <div style='text-align:center; border-bottom:1px solid #3a6a3a; padding-bottom:6px; margin-bottom:6px;'>
                    <b style='color:#5ceb9a'>DEBIT</b>
                </div>
            """, unsafe_allow_html=True)
            for e in entries:
                if e['debit'] > 0:
                    st.markdown(f"<div style='text-align:left; font-size:0.9em; padding:2px 0;'>{e['ref']}: Rp {e['debit']:,.0f}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='border-top:1px solid #3a6a3a; padding-top:4px; margin-top:4px;'><b>Total: Rp {total_debits:,.0f}</b></div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

        with col_t2:
            st.markdown(f"""<div style='background:#2a1a1a; border:1px solid #6a3a3a; border-radius:8px; padding:12px; margin-bottom:8px;'>
                <div style='text-align:center; border-bottom:1px solid #6a3a3a; padding-bottom:6px; margin-bottom:6px;'>
                    <b style='color:#ff6b6b'>CREDIT</b>
                </div>
            """, unsafe_allow_html=True)
            for e in entries:
                if e['credit'] > 0:
                    st.markdown(f"<div style='text-align:left; font-size:0.9em; padding:2px 0;'>{e['ref']}: Rp {e['credit']:,.0f}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='border-top:1px solid #6a3a3a; padding-top:4px; margin-top:4px;'><b>Total: Rp {total_credits:,.0f}</b></div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

        balance_side = "Dr" if end_balance >= 0 else "Cr"
        balance_color = "#5ceb9a" if end_balance >= 0 else "#ff6b6b"
        st.markdown(f"""<div style='text-align:center; background:linear-gradient(135deg, #1a1d2a, #252840); border:1px solid #3a3f5c; border-radius:10px; padding:12px; margin-top:8px;'>
            <span style='color:#999'>Ending Balance:</span>
            <span style='color:{balance_color}; font-size:1.3em; font-weight:bold; margin-left:8px;'>Rp {abs(end_balance):,.0f} {balance_side}</span>
        </div>""", unsafe_allow_html=True)
...
def render_trial_balance():
    ...
    st.markdown("View the trial balance — a summary of all account balances before adjustment.")
    ...
    balances = {}  # account_name -> net balance (debit - credit)
    account_types = {}
    ...
    for entry in filtered:
        for line in entry["entries"]:
            ...
            if acc not in balances:
                balances[acc] = 0
                account_types[acc] = get_account_type(acc, company_type)
            balances[acc] += line["debit"] - line["credit"]

    # Sort by classification order
    class_order = {"Assets": 0, "Liabilities": 1, "Equity": 2, "Revenue": 3, "COGS": 4, "Expenses": 5, "Other": 6}
    sorted_accounts = sorted(balances.keys(), key=lambda a: (class_order.get(account_types.get(a, "Other"), 99), a))
    ...
    for acc in sorted_accounts:
        ...
        normal = get_account_normal_balance(account_types.get(acc, "Other"))

        # In trial balance: debit balances go in debit column, credit balances in credit column
        if balance >= 0:
            debit_bal = balance
            credit_bal = 0
        else:
        ...
        tb_rows.append({
            "Account": acc,
            "Type": account_types.get(acc, ""),
            "Normal": normal,
            "Debit (Rp)": f"Rp {debit_bal:,.0f}" if debit_bal > 0 else "",
            "Credit (Rp)": f"Rp {credit_bal:,.0f}" if credit_bal > 0 else "",
        })

    # Display
    st.info(f"**Company Type:** {company_type} | **Total Accounts:** {len(tb_rows)}")

    # Filter by account type
    type_filter = st.selectbox("Filter by Account Type", ["All"] + sorted(set(account_types.values())))
    if type_filter != "All":
        tb_rows = [r for r in tb_rows if r["Type"] == type_filter]
    ...
    with col_tb1:
        st.markdown(f"""<div class="result-card">
            <h4>Total Debit</h4>
            <div class="result-value">Rp {total_debit:,.0f}</div>
        </div>""", unsafe_allow_html=True)
    with col_tb2:
        st.markdown(f"""<div class="result-card">
            <h4>Total Credit</h4>
            <div class="result-value">Rp {total_credit:,.0f}</div>
        </div>""", unsafe_allow_html=True)
    with col_tb3:
        balanced = abs(total_debit - total_credit) < 1
        st.markdown(f"""<div class="result-card">
            <h4>Status</h4>
            <div class="result-value" style="color:{"#5ceb9a" if balanced else "#ff6b6b"};">
    ...
    if tb_rows:
        fig = go.Figure()
        accounts_label = [r["Account"] for r in tb_rows]
        debits_plot = [r["Debit (Rp)"] for r in tb_rows]
        credits_plot = [r["Credit (Rp)"] for r in tb_rows]
        ...
        for r in tb_rows:
            d = r["Debit (Rp)"]
            c = r["Credit (Rp)"]
            debit_vals.append(float(d.replace("Rp ", "").replace(",", "")) if d else 0)
            credit_vals.append(float(c.replace("Rp ", "").replace(",", "")) if c else 0)

        fig.add_trace(go.Bar(name="Debit", x=accounts_label, y=debit_vals, marker_color="#5ceb9a"))
        fig.add_trace(go.Bar(name="Credit", x=accounts_label, y=credit_vals, marker_color="#ff6b6b"))
        fig.update_layout(barmode="group", height=400,
                          plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                          font_color='#ccc', xaxis_title="Account", yaxis_title="Amount (Rp)")
        st.plotly_chart(fig, width="stretch")
